        ...
        <div className="bg-white dark:bg-white/5 rounded-xl border border-[#f1f3f1] dark:border-white/10 shadow-sm flex flex-col h-full">
                ...
                {ingredients.map((it: DashboardIngredient) => (
                    <div key={it.id} className="flex items-center gap-4 bg-white dark:bg-transparent px-6 py-4 border-b border-[#f1f3f1] dark:border-white/5 hover:bg-background-light dark:hover:bg-white/5 transition-colors group">
                        <div className="text-primary flex items-center justify-center rounded-lg bg-primary/10 shrink-0 size-10">
                            <span translate="no" className="material-symbols-outlined notranslate text-xl">{it.name === 'Pechuga pollo' ? 'restaurant' : it.name === 'Jitomate' ? 'nutrition' : 'cooking'}</span>
                        </div>
                        ...
                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => onEdit(it.id)} className="p-1.5 hover:text-primary transition-colors"><span translate="no" className="material-symbols-outlined notranslate text-xl">edit</span></button>
                            <button onClick={() => onDelete(it.id)} className="p-1.5 hover:text-red-500 transition-colors"><span translate="no" className="material-symbols-outlined notranslate text-xl">delete</span></button>
                        </div>
                    </div>
                ))}
            </div>
            <div className="p-6">
                <button onClick={onAdd} className="w-full flex items-center justify-center gap-2 bg-primary text-white py-3 rounded-full font-bold shadow-md hover:shadow-lg transition-all active:scale-[0.98]">
                    <span translate="no" className="material-symbols-outlined notranslate">add</span>
